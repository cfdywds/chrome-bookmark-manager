...
- Chrome 或 Microsoft Edge 114 及以上版本。
- 以「加载已解压的扩展程序」方式使用；当前不通过 Chrome Web Store 分发。
- 开发与 CI 使用 Node.js 20；扩展运行时不依赖 Node.js 或构建步骤。
...
首次公开发布使用 `v1.0.0` 作为初始 tag；之后按语义化版本递增。发布 tag 和 GitHub Release 是两个独立步骤，前者用于版本校验，后者用于向用户提供下载入口。首次发布后再运行 `npm run check-version`；该命令会校验最新发布 tag。
...
| git tag | 使用 annotated tag，格式为 `v1.0.1` |
...
npm run release -- 1.0.1                     # 指定版本，仍逐步确认
npm run release -- 1.0.1 --push              # 指定版本并在确认后推送当前分支和 tag
npm run release -- 1.0.1 --push --yes        # 自动确认；仅用于 CI 等受控环境
npm run release -- 1.0.1 --push --allow-legacy-invalid-tags # 仅在确认远端最新 tag 是历史遗留异常时使用
```
...
# ② 同步所有版本文件并提交
# manifest.json / package.json / package-lock.json：1.0.1
git add -A && git commit -m "release: v1.0.1"
# ③ 在刚提交的 HEAD 打 annotated tag，随后校验
git tag -a v1.0.1 -m "v1.0.1: <更新摘要>" HEAD
git rev-parse v1.0.1^{commit}
git show v1.0.1:manifest.json
# ④ 推送分支与 tag
git push origin main
git push origin v1.0.1
# ⑤ GitHub Release（网页）
# 仓库 → Releases → Draft a new release
# → 选择既有 tag v1.0.1，并核对其 commit → 标题/说明 → Publish release
```
...
- 回收站（30 天软删除 / 恢复 / 自动清理）
- AI 打标与重试（避免 token 超限）
- 标签体系（多标签 / 标签池 / 收敛 / 管理）
- 备份 v2（含全部自定义数据，id 映射恢复）
- New Tab 新标签页（标签分组 + 搜索 + 卡片操作）
- ⭐ 收藏按钮接管 + 图标切换侧边栏
- 隐藏书签（全视图排除 + 实时同步）
