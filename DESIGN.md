...
- newtab 文件夹筛选使用自定义可访问 listbox，保留上下键、Home/End、Enter 和 Esc 操作，选项使用缩进引导线、文件夹图标与胶囊计数表达层级，并在加载期间显示状态；列表视图按 favicon、主信息、标签状态、操作区分栏，标签最多展示一项加余数，操作区按需显出，触屏设备保持常驻。
