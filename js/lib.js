...
(function (global) {
  ...
  async function hasLlmHostPermission(rawBaseUrl) {
    ...
    if (!global.chrome || !global.chrome.permissions || !global.chrome.permissions.contains)
      return true;
  ...
  async function requestLlmHostPermission(rawBaseUrl) {
    const origin = getLlmHostPermission(rawBaseUrl);
    if (!global.chrome || !global.chrome.permissions || !global.chrome.permissions.request)
      return true;
  ...
  const CATEGORY_MATCHERS = CATEGORY_RULES.map(
    rule => new RegExp(rule.keys.map(key => escapeRegExp(key.toLowerCase())).join('|'), 'u')
  );
  ...
  const LOGIN_HOST_LABELS = new Set([
    'login',
    'signin',
    'sign-in',
    'auth',
  ...
  const FINANCIAL_HOST_LABELS = new Set([
    'bank',
    'banking',
    'paypal',
    'alipay',
  ...
  const FINANCIAL_TITLE_PATTERN =
    /(网上银行|银行账户|支付账户|证券账户|加密钱包|数字钱包|crypto wallet)/i;
  ...
  function isFinancialOrWalletService(url, title) {
    const hostLabels = url.hostname.toLowerCase().split('.');
    return (
      hostLabels.some(label => FINANCIAL_HOST_LABELS.has(label)) ||
      FINANCIAL_TITLE_PATTERN.test(title || '')
    );
  }
  ...
  const SENSITIVE_RULES = [
    ...
    { label: '含访问凭据参数', sev: 'high', test: url => hasSensitiveUrlParameter(url) },
    {
      label: '金融 / 钱包服务',
      sev: 'high',
      test: (url, title) => isFinancialOrWalletService(url, title)
    }
  ];
  ...
  function detectSensitive(host, rawUrl, title) {
    ...
    return SENSITIVE_RULES.filter(rule => rule.test(url, title || host || '')).map(rule => ({
      label: rule.label,
      sev: rule.sev
    }));
  }
  ...
  async function loadDomainGroups() {
    ...
    try {
      ...
      domainGroupsCache = Object.fromEntries(
        Object.entries(rules.domain).flatMap(([domain, tags]) => {
          // 与实际打标相同：只有固定池内的有效标签才能成为概览分类。
          const category = (Array.isArray(tags) ? tags : [])
            .map(tag =>
              fixedTags.find(
                poolTag =>
                  String(poolTag).toLowerCase() === String(tag).toLowerCase() &&
                  poolTag !== FALLBACK_TAG
              )
            )
            .find(Boolean);
  ...
  function invalidateDomainGroups() {
    domainGroupsCache = null;
  }
  ...
  function applyTagChangesLocally(map, changes, mode) {
    ...
    Object.entries(changes || {}).forEach(([id, value]) => {
      ...
      if (mode === 'merge' && Array.isArray(value))
        next = [...new Set([...current, ...value])].slice(0, MAX_TAGS_PER_BOOKMARK);
  ...
  function inferDomainTags(item, poolOverride) {
    let host = String((item && item.host) || '')
      .toLowerCase()
      .replace(/^www\./, '');
    ...
    if (!host) return [];
    const pool =
      Array.isArray(poolOverride) && poolOverride.length
        ? poolOverride
        : fixedTagsCache || [...DEFAULT_FIXED_TAGS, FALLBACK_TAG];
    const tags = [];
    const add = name => {
      const found = pool.find(tag => String(tag).toLowerCase() === name.toLowerCase());
      if (found && found !== FALLBACK_TAG && !tags.includes(found) && tags.length < 3)
        tags.push(found);
    };
  ...
  async function setTags(id, tags) {
    if (!id) return false;
    await loadFixedTags();
    const clean = [...new Set((tags || []).map(t => normalizeToPool(t)).filter(Boolean))].slice(
      0,
  ...
  async function setTagsBatch(changes, mode) {
    ...
    Object.entries(changes || {}).forEach(([id, tags]) => {
      ...
      const clean = [
        ...new Set((tags || []).map(tag => normalizeToPool(tag)).filter(Boolean))
  ...
  function isClosedNativeSyncChannel(error) {
    return (
      !(error instanceof NativeSyncResponseError) &&
  ...
  function nativeSyncTransportErrorKind(status) {
    ...
    if (
      ...
      LEGACY_CLOSED_NATIVE_SYNC_CHANNEL.test(String(status.lastError))
    ) {
  ...
  async function setTagSyncEnabled(enabled) {
    ...
    try {
      ...
      /* storage 事件和 Service Worker 启动恢复仍会处理该持久化请求 */
    }
    return target;
  }
  ...
  async function forceResyncTags() {
    try {
      const result = await requestNativeTagSync('forceResync');
      return !!result.ok;
    } catch (e) {
      if (isClosedNativeSyncChannel(e)) {
        await clearClosedNativeSyncStatus();
  ...
  function suggestTags(item) {
    ...
    // 只保留池内且非「其他」的建议
    return [...new Set(tags.map(t => normalizeToPool(t)))]
      .filter(t => t && t !== FALLBACK_TAG)
      .slice(0, MAX_TAGS_PER_BOOKMARK);
  }
  ...
  function invalidateHiddenIds() {
    hiddenCache = null;
  }

  // 同步判断（可能未加载，返回 false）
  function isHidden(id) {
    return hiddenCache ? hiddenCache.has(id) : false;
  }
  ...
  async function toggleHidden(id) {
    ...
    try {
      await chrome.storage.local.set({ [HIDDEN_KEY]: [...hiddenCache] });
    } catch (e) {
      /* ignore */
    }
  ...
  function getBookmarkMetadata(rawUrl, title) {
    ...
    try {
      ...
      /* 与 urlKey / routeKeyOf 的无效 URL 回退一致 */
    }
  ...
  function parseAiCategories(content, items, validSet) {
    ...
    try {
      ...
      throw new Error('无法解析 LLM 返回的 JSON：' + txt.slice(0, 120));
    }
  ...
  function isWebUiBaseUrl(base) {
    try {
      ...
      if (
        /(chatgpt\.com|claude\.ai|gemini\.google\.com|perplexity\.ai|poe\.com|aistudio\.google\.com)$/i.test(
          host
        )
      )
        return true;
  ...
  async function parseJsonResp(resp, ctx, baseUrl) {
    ...
    const webHint =
      baseUrl && isWebUiBaseUrl(baseUrl)
    ...
    if (ct.includes('text/html') || ct.includes('text/plain')) {
      let snippet = '';
      try {
        snippet = (await resp.text()).replace(/\s+/g, ' ').slice(0, 100);
      } catch (e) {
        /* noop */
      }
      throw new Error(
        ctx +
          '返回了 HTML 页面而非 JSON（HTTP ' +
          resp.status +
          '）' +
          shown +
          '。' +
  ...
  function modelEndpointCandidates(rawBaseUrl, provider) {
    ...
    const add = (url, auth) => {
      ...
      if (!endpoints.some(item => item.key === key))
        endpoints.push({ url, auth: auth || 'bearer', key });
    };
    let parsed;
    try {
      parsed = new URL(base);
    ...
    const entries = Array.isArray(data)
      ...
      : data && Array.isArray(data.data)
        ...
        : data && Array.isArray(data.models)
          ? data.models
          : [];
    return [...new Set(entries.map(modelNameFromEntry).filter(Boolean))].sort((left, right) =>
  ...
  async function listModels(cfg) {
    ...
    for (const endpoint of endpoints) {
      ...
      if (!resp.ok) {
        ...
        lastError = new Error(
          '端点 ' + endpoint.url + ' 返回 HTTP ' + resp.status + (detail ? '：' + detail : '')
        );
  ...
  async function chatWithFallback(body, cfg) {
    ...
    for (const ep of endpoints) {
      ...
      lastErr = new Error(
        '端点 ' + ep + ' 返回 HTTP ' + resp.status + (ct.includes('html') ? '（HTML 页面）' : '')
      );
      if (resp.status === 401 || resp.status === 403) break; // 认证错误换端点无用
    }
    if (!resp) {
      throw new Error(
        '网络请求失败（可能是 Base URL 错误或 CORS/网络问题）：' +
    ...
    if (isWebUiBaseUrl(base)) {
      throw new Error(
        '你填的 Base URL（' +
          base +
          '）疑似「网页版」地址（chat./platform./console. 前缀）。请改为 API 端点，例如：\n· DeepSeek: https://api.deepseek.com/v1\n· OpenAI: https://api.openai.com/v1'
      );
    }
    let detail = '';
    try {
  ...
  async function aiClassifyBatched(items, cfg, opts) {
    ...
    for (let i = 0; i < total; i += batchSize) {
      ...
      if (opts.onProgress) {
        opts.onProgress(
          Math.min(i + batch.length, total) / total,
  ...
  function parseSuggestedTags(content) {
    ...
    if (Array.isArray(parsed)) list = parsed;
      ...
      const key = ['tags', 'results', 'labels', 'categories', 'pool'].find(k =>
    ...
    list.forEach(item => {
      const raw =
        typeof item === 'string' ? item : (item && (item.tag || item.name || item.label)) || '';
  ...
  async function suggestFixedTags(samples, cfg) {
    ...
    const user =
      ...
      list
        .map(
          (s, i) =>
            `${i + 1}. ${s.domain} | ${s.count} | ${(s.titles || []).join(' / ').slice(0, 120)}`
        )
        .join('\n');
    const body = {
      model: cfg.model,
  ...
  async function aiTag(items, cfg) {
    ...
    eligibleItems.forEach(item => {
      ruleTags[String(item.id)] = inferHighConfidenceTags(item, pool);
    });
    const system = [
  ...
  function parseAiTags(content, items) {
    ...
    const collect = (id, tags) => {
      ...
      const clean = [...new Set((arr || []).map(t => normalizeToPool(t)).filter(Boolean))].slice(
        0,
        MAX_TAGS_PER_BOOKMARK
      );
    ...
    if (Array.isArray(arr)) {
      arr.forEach(r => {
        if (r && r.id != null) collect(r.id, r.tags != null ? r.tags : r.tag);
      });
    } else if (obj && typeof obj === 'object') {
      Object.keys(obj).forEach(k => {
        if (k !== 'results' && k !== 'data') collect(k, obj[k]);
      });
    }
    const ids = new Set(items.map(i => String(i.id)));
    Object.keys(map).forEach(k => {
  ...
  async function restoreTrashItems(items, opts) {
    ...
    const requested = requestedIds.size
      ? [...requestedIds]
      : (await getTrash()).map(item => item.id);
    ...
    if (typeof opts.onProgress === 'function' && total) {
      try {
        opts.onProgress({ done: total, total, restored: result.restored || 0 });
      } catch (e) {
        /* 进度回调失败不影响恢复 */
      }
    }
  ...
  async function purgeExpiredTrash() {
    try {
      ...
      if (
        /Receiving end does not exist|message (?:channel|port) closed|asynchronous response.*channel closed/i.test(
  ...
  function decodeBackupTrashRecords(records, exportedAt) {
    ...
    (records || []).forEach(raw => {
      ...
      let url;
      try {
        url = normalizeHttpUrl(raw[2]).href;
      } catch (e) {
        return;
      }
  ...
  async function ensureBackupTrashCapacity(records, exportedAt) {
    const current = await getTrash();
    const ids = new Set(current.map(item => String(item.id)));
    const required = decodeBackupTrashRecords(records, exportedAt).filter(
  ...
  async function importBookmarksJSON(json, opts) {
    ...
    const topLevelFoldersByRoot = new Map(
      targetRoots.map(root => [String(root.id), indexTopLevelFolders(root.children)])
    );
    ...
    function restoreBookmarkMetadata(node, bookmarkId) {
      if (node.tags.length) {
        ...
        if (tags.length)
          restoredTags[bookmarkId] = [...new Set([...(restoredTags[bookmarkId] || []), ...tags])];
      }
    ...
    async function walk(nodes, parentId, depth) {
      for (const raw of nodes || []) {
        ...
        if (node.type === 'bookmark') {
          let url;
          try {
            url = normalizeHttpUrl(node.url).href;
          } catch (e) {
            stats.skipped++;
            continue;
          }
        ...
        if (depth === 0) {
          ...
          if (dup) {
            pid = dup.id;
            if (node.sourceId) folderIdMap.set(node.sourceId, pid);
            stats.reused++;
          } else {
            try {
              const f = await chrome.bookmarks.create({
                parentId,
    ...
    for (const rawRoot of data.roots) {
      ...
      if (!target) {
        stats.skipped += countBackupBookmarks(root.children);
        continue;
      }
    ...
    if (!dryRun) {
      ...
      if (Object.keys(restoredTags).length) {
        try {
          await mergeTagsBatch(restoredTags);
        } catch (e) {
          /* ignore */
        }
      }
