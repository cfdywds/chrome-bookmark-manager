...
- 新标签页的光标落点改为「按一次 Tab 即进搜索框」：Chrome 会把新标签页加载时的键盘焦点强制留给地址栏，并忽略页面发出的 `focus()` 与 `autofocus`（浏览器进程层策略，扩展无法绕过，因此「打开即可打字」做不到），所以搜索框声明 `tabindex="1"` 成为页面首个 Tab 目标，按一次 Tab 离开地址栏光标就落在搜索框（点页面空白处同样），页面拿到键盘焦点时再补一次聚焦兜住 Tab 默认的焦点移动；`autofocus` 与 0～4s 时间窗重试保留，用于页面在加载时就已持有焦点的情况（例如扩展自己创建的标签页），框内已有内容时全选，用户开始操作后不再抢焦点。
- 管理面板（侧边栏 / 独立标签页）打开即把光标放进顶部搜索框：`autofocus` 加 JS 在 0～1.2s 时间窗内重试聚焦（Chrome 在面板刚加载时可能仍把焦点留在浏览器 UI，首次 `focus()` 会被忽略），用户点击或按键后立即停止；侧边栏文档被复用、不重跑初始化时，再次可见会重新武装。代价是打开面板后按 `j`/`k` 会先被搜索框接收，方向键导航不受影响。
- 侧边栏申请 `activeTab` 权限以读取当前页地址；不申请 `tabs`，避免浏览历史警告和扩展更新时被禁用等待重新同意。
- 面板「标签」页的自定义规则摘要行新增「自定义规则」入口（未配置规则的空态同样有）：点击直接跳到设置页的「标签体系 → 自定义规则」并滚到该分组。设置页已打开时复用该标签页、只改 hash 不重载（不丢未保存的编辑），查询已开设置页走 `runtime.getContexts`，因此仍不需要 `tabs` 权限。
- 设置页「标签体系 → 固定标签池」新增「AI 初始化」：先在本地按域名聚合出书签最多的 80 个站点（每站带 2 条标题样本），再让 AI 归纳 15～25 个分类标签（现有池作为参考，仍然适用的沿用原词），结果先预览，由用户选「覆盖标签池」或「并入现有池」后写入。只发送域名与标题样本，内部同步目录、非网页书签与登录 / 金融类高敏书签都不会发送；未配置 AI 时先引导到 AI 分组。
...
- 修复扩展在 `chrome://extensions` 重新加载（或更新）后，从新标签页入口打开侧边栏只剩一片空白（连顶栏都不存在）的问题：重载会让 Chrome 丢掉 `side_panel.default_path` 注册并销毁面板已加载的文档，此时 `sidePanel.open()` 只露出空壳，面板内部的看门狗与错误卡都没有机会运行。后台改为在 SW 启动与 `onInstalled` 时写回面板路径，新标签页入口在用户手势内先写回路径再 `open()`，失败时降级为以标签页打开管理界面，顶栏另加一个不经过侧边栏的常驻入口；窄屏（≤560px）下「打开管理面板」与「开源仓库」一起收成纯图标。
- 修复隐藏书签后过了一会儿又自己出现的问题：原生同步水合（hydrate）此前会把远端记录里的「未隐藏」无条件写回本机，等于用还没更新的旧记录抹掉用户刚做的隐藏。现在远端记录只允许「补上」隐藏，取消隐藏仅在本机记录确实写过该 URL 隐藏、且其修订号不比本机记录旧时才生效；写回隐藏集合前还会重读一次本机最新值，把用户在同步窗口内新做的隐藏合并进来。
