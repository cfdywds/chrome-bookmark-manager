...
| 文件 | 作用 |
|---|---|
| `popup.html` + `js/popup.js`（4891 行）+ `css/popup.css`（3601 行） | 侧边栏管理面板 |
| `newtab.html` + `js/newtab.js`（1496 行）+ `css/newtab.css`（1538 行） | 新标签页 |
| `options.html` + `js/options.js` | 设置页 |
| `css/tokens.css`（332 行） | **设计令牌唯一来源**，三页共用 |
...
03 原子件               304-643
04 复合组件             644-1631
05 空/加载/错误态        1632-1791
06 概览页               1792-2167
07 组织页               2168-2821
08 选项页               2822-3393
09 键盘/拖拽/动效        3394-3601
```
...
> 最后更新：**P0 / P1 / P2 / P3 块 1 完成**。`npm test` 15 文件 397 测试全绿，`npm run lint` 0 错。
...
- **剩余人工走查（唯一待办）**：自动化能覆盖的部分（双主题截图、对比度核算、360px 溢出量测、全量测试）已随各块完成并通过。还需要人来做的只有两件事：
