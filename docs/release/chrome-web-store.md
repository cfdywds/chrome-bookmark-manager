...
2. 打开设置页，开启「标签原生同步」。扩展会在 Chrome 的「其他书签」下创建 `书签管家同步数据（请勿修改）`，并把当前标签、隐藏状态、固定标签池和自定义规则写入该目录。
3. 在每台其他设备打开 `chrome://extensions`，开启开发者模式，选择「加载已解压的扩展程序」，并选择本项目的任意最新版本地副本。无需比较扩展 ID。
4. 等待 Chrome 完成书签同步，打开侧边栏、新标签页或设置页。扩展会自动发现内部目录并读取标签、隐藏状态和规则；不要在其他设备重复导入备份。
...
- 备份与标签原生同步均不包含 API Key、LLM 配置、端点、模型或可选主机权限；标签原生同步不包含回收站。
