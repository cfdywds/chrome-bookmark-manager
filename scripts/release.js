...
async function selectVersion() {
  ...
  if (skipPrompts) fail('--yes 必须同时提供版本号，例如：npm run release -- 1.3.1 --yes');
...
function showHelp() {
  ...
  console.log('  npm run release -- 1.3.1                # 指定版本，仍会交互确认');
  console.log('  npm run release -- 1.3.1 --push         # 指定版本并在确认后推送');
  console.log('  npm run release -- 1.3.1 --push --yes   # 自动确认；仅在 CI 等受控环境使用');
  console.log('  npm run release -- 1.3.1 --push --allow-legacy-invalid-tags  # 仅在核实远端最新 tag 是历史遗留异常时使用');
